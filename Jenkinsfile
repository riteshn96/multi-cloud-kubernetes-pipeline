                        ...
                        script {
                            docker.image('alpine/k8s:1.27.5').inside {
                                withCredentials([aws(credentialsId: 'aws-credentials')]) {
                                    ...
                                    // 1. Install AWS CLI in the agent
                                    ...
                                    // 3. FIX KUSTOMIZE COMMAND
                                    //    We navigate into the directory first, then run the command.
                        ...
                        script {
                            docker.image('alpine/k8s:1.27.5').inside {
                                withCredentials([azureServicePrincipal(credentialsId: 'azure-credentials')]) {
                                    ...
                                    // 1. Install Azure CLI using the Alpine package manager
                                    sh 'apk add --no-cache azure-cli'

                                    // 2. Login and connect kubectl
